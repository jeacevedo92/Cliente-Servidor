...
static string terminalAddress = "tcp://";
...
void handleClientRequest(json &req) {
	...
	} else if (req["source"] == "terminal") {
		if (req["op"] == "send keys range")
		{
			socket terminal(ctx, socket_type::push);
			terminal.connect(terminalAddress);
			json send_data_terminal = {
		    	{"op", "range"},
				{"my_id", upperBound},
				{"rng", lowerBound}
		    };
		    terminal.send(send_data_terminal.dump());
		    terminal.close();
		}
	} else {
